...
export function getOrCreateTenantProfile(tenantId: string): SandboxTenantProfile {
  ...
  const profile: SandboxTenantProfile = {
    tenantId,
    displayNameEn: tenantId === 'tnt_demo' ? 'Partnora Demo Bank' : `Tenant ${tenantId}`,
    displayNameAr: tenantId === 'tnt_demo' ? 'بنك بارتنورا التجريبي' : `المستأجر ${tenantId}`,
  };
  tenantProfiles.set(tenantId, profile);
  return profile;
}

export function generateApiKeySecret(prefix = 'partnora_demo'): string {
